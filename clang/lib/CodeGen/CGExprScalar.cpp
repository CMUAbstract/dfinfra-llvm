...
Value *ScalarExprEmitter::VisitSpawnExpr(SpawnExpr *E) {
    ...
    // 1. Define the function type explicitly.
    // "spawn" usually has the signature: ResultTy (PtrTy, ...)
    // The 'true' at the end enables variadic arguments (varargs), which spawn requires.
    llvm::FunctionType *FnType = llvm::FunctionType::get(ResultTy, {PtrTy}, /*isVarArg=*/true);

    // 2. Insert it as a regular function named "spawn" (or any name NOT starting with "llvm.")
